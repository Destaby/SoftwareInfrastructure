...
import { storage as storageDecorator } from '../database/mongodb/decorators';
...
export class StudentsStorage extends Storage<Student> {
  constructor(@storageDecorator(Student) protected storage: IStorage<Student>) {
    super();
  }
}

export default (connection: MongooseConnection) =>
  new StudentsStorage(new MongooseStorage(connection, Student));
